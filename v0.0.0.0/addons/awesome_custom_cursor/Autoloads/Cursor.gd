...
func set_cursor(cursor_name: String) -> void:
	if !sprite or !sprite.sprite_frames:
		push_error("sprite 或 sprite_frames 尚未初始化")
		return

	if sprite.sprite_frames.has_animation(cursor_name):
		sprite.play(cursor_name)
		sprite.offset = get_offset()
	else:
		push_warning("未找到动画: %s" % cursor_name)
